...
## Results on real 4-vCPU hardware (GitHub Actions CI, shm transport)

| | C++ mean | Python mean |
|---|---|---|
| imu (100Hz) | 0.0007ms | 0.5386ms |
| encoder (50Hz) | 0.0008ms | 0.5410ms |
| pose (20Hz) | 0.0009ms | 0.5528ms |

Zero drops on either side. The gap **widens** to roughly **600-770x**
on real multi-core hardware, not narrows -- consistent with this
project's established finding elsewhere (the C++ port report's
multi-core investigation): extra cores let C++'s publisher and
subscriber genuinely run concurrently instead of time-slicing one
core, while Python's GIL still serializes bytecode execution
regardless of core count, so multi-core hardware helps the native
implementation far more than the interpreted one. Current numbers are
always in `commsys/results/cpp_report.md` and `commsys/results/
python_report.md`, regenerated by CI on every full benchmark run.
